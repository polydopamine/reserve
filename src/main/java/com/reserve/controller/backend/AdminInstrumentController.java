package com.reserve.controller.backend;

import com.github.pagehelper.PageInfo;
import com.reserve.common.Const;
import com.reserve.common.ServerResponse;
import com.reserve.pojo.Instrument;
import com.reserve.pojo.Lab;
import com.reserve.pojo.User;
import com.reserve.service.IInstrumentService;
import com.reserve.vo.InstrumentVo;
import net.sf.jsqlparser.schema.Server;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;

import javax.servlet.http.HttpSession;

@Controller
@RequestMapping("/admin/inst/")
public class AdminInstrumentController {

    @Autowired
    private IInstrumentService iInstrumentService;

    @RequestMapping(value = "list.do")
    @ResponseBody
    // 返回课题组列表
    public ServerResponse<PageInfo> list(HttpSession session, @RequestParam(value = "pageNum", defaultValue = "1") int pageNum, @RequestParam(value = "pageSize", defaultValue = "10") int pageSize) {
        User user = (User) session.getAttribute(Const.CURRENT_USER);
        if (user.getEnableRole() == Const.EnableRole.ROLE_ADMIN || user.getEnableRole() == Const.EnableRole.ROLE_SUPER_ADMIN) {
            return ServerResponse.createByErrorMessage("没有管理员权限");
        }
        return iInstrumentService.list(user, pageNum, pageSize);
    }

    @RequestMapping(value = "add.do", method = RequestMethod.POST)
    @ResponseBody
    // 新增课题组的信息
    public ServerResponse add(HttpSession session, Instrument instrument) {
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser.getEnableRole() != Const.EnableRole.ROLE_ADMIN && currentUser.getEnableRole() != Const.EnableRole.ROLE_SUPER_ADMIN) {
            return ServerResponse.createByErrorMessage("没有管理员权限");
        }
        return iInstrumentService.add(instrument);
    }

    @RequestMapping(value = "update.do", method = RequestMethod.POST)
    @ResponseBody
    // 对课题组的信息进行修改
    public ServerResponse update(HttpSession session, Instrument instrument) {
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser.getEnableRole() != Const.EnableRole.ROLE_ADMIN && currentUser.getEnableRole() != Const.EnableRole.ROLE_SUPER_ADMIN) {
            return ServerResponse.createByErrorMessage("没有管理员权限");
        }
        return iInstrumentService.update(instrument);
    }

    @RequestMapping(value = "delete.do")
    // 对课题组的信息进行修改
    public String delete(HttpSession session, Integer instId) {
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser.getEnableRole() != Const.EnableRole.ROLE_ADMIN && currentUser.getEnableRole() != Const.EnableRole.ROLE_SUPER_ADMIN) {
            return "portal/error-page";
        }
        ServerResponse response = iInstrumentService.delete(instId);
        return "backend/instrument-list";
    }

    @RequestMapping(value = "detail.do")
    @ResponseBody
    // 删除课题组的信息
    public ServerResponse<InstrumentVo> detail(HttpSession session, Integer instrumentId) {
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser.getEnableRole() != Const.EnableRole.ROLE_ADMIN && currentUser.getEnableRole() != Const.EnableRole.ROLE_SUPER_ADMIN) {
            return ServerResponse.createByErrorMessage("没有管理员权限");
        }
        return iInstrumentService.detail(instrumentId);
    }
}
